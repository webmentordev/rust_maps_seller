        ...
        <div class="max-w-5xl m-auto pt-[150px] pb-12 px-4 min-h-screen">
            <div class=" border border-white/10 p-12 rounded-lg bg-dark-100">
                ...
                <div class="flex items-center mb-3">
                    <form action="{{ route('client.filter') }}" method="get" class="mr-3">
                        ...
                        @foreach ($orders as $order)
                            <div class="pl-4 pr-6 py-3 rounded-lg bg-dark border border-white/10 mb-3 flex items-center justify-between">
                                <div class="flex items-center">
                                    <a target="_blank" href="{{ asset('/storage/'. $order->product->thumbnail) }}" class="p-1 rounded-full border hover:border-rust-green border-rust overflow-hidden">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <a href="{{ route('report') }}" class="ml-4 px-4 text-lg link py-1 text-white bg-rust hover:bg-rust-green transition-all">Report Bug</a>
                                    <form action="{{ route('client.download', $order->map_slug) }}" method="post">
                                        @csrf
                                        <button type="submit" class="ml-4 px-4 text-lg link py-1 text-white bg-rust hover:bg-rust-green transition-all">Download</button>
                                    </form>
                                </div>
                            </div>
                        @endforeach
